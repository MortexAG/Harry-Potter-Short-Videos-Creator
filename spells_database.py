...
cursor.execute('''
    CREATE TABLE IF NOT EXISTS stored_spells (
...
def insert_key_value(spell, description):
    cursor.execute('INSERT INTO stored_spells VALUES (?, ?)', (spell, description))
    conn.commit()
...
def get_value(spell):
    cursor.execute('SELECT description FROM stored_spells WHERE spell = ?', (spell,))
...
def get_random_key_value():
    cursor.execute('SELECT spell, description FROM stored_spells ORDER BY RANDOM() LIMIT 1')
    result = cursor.fetchone()
    if result:
        return result
    else:
...
def remove_spell_from_database(spell):
    ...
    cursor.execute("DELETE FROM stored_spells WHERE spell = ?", (spell,))
    conn.commit()
